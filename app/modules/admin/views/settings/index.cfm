...
<!---
	...
	Unlike the page editor's tabs these panels hold *separate* forms, one per
	action, because each posts to its own endpoint and answers to its own
	permission. Saving one does not touch the others, which is why each panel
	keeps its own button.

	The tabs are CSS-only and every panel stays in the DOM, so switching tabs
	cannot lose what has been typed. The radios are matched to panels by
	position; see the note in `_styles.cfm` before adding one.

	A panel whose permission the user lacks is still shown, and says so. A tab
	that silently disappeared would leave someone wondering where a setting had
	gone, and the tab positions have to stay stable for the CSS to line up.
--->
<div class="tabs">
	<input type="radio" name="settingsTab" id="tab-site" checked>
	<label for="tab-site">Site</label>

	<input type="radio" name="settingsTab" id="tab-branding">
	<label for="tab-branding">Branding</label>

	<input type="radio" name="settingsTab" id="tab-theme">
	...
	<input type="radio" name="settingsTab" id="tab-domains">
	<label for="tab-domains">Domains</label>


		<section class="tab-panel" data-for="tab-site">
		...
		<section class="tab-panel" data-for="tab-branding">
			...
			<div class="grid-2">
				<div>
					...
					<input type="text" id="colorPrimary" name="colorPrimary"
					       ...
					       placeholder="##0f2a4a" #prc.canSettings ? '' : 'disabled'#>
				</div>
				<div>
					...
					<input type="text" id="colorAccent" name="colorAccent"
					       ...
					       placeholder="##c19b43" #prc.canSettings ? '' : 'disabled'#>
				</div>
			</div>
			...
			<div class="grid-2">
				<div>
					...
					<input type="text" id="fontHeading" name="fontHeading"
					       ...
					       placeholder="'Source Serif 4', Georgia, serif" #prc.canSettings ? '' : 'disabled'#>
				</div>
				<div>
					...
					<input type="text" id="fontBody" name="fontBody"
					       ...
					       placeholder="Inter, system-ui, sans-serif" #prc.canSettings ? '' : 'disabled'#>
				</div>
			</div>
			<p class="muted">
		...
		*/
		...
		<form method="post" action="/admin/settings/theme">
			...
			<select id="theme" name="theme" #prc.canSettings ? '' : 'disabled'#>
		...
		<section class="tab-panel" data-for="tab-recaptcha">
		<cfif prc.canSettings>
		...
		<section class="tab-panel" data-for="tab-domains">
		<table>

<cfscript>
	// Interactive "Areas We Serve" split: services on the left, that service's
	// locations on the right. Built from the live page tree so a new suburb or
	// service page appears here without editing this file.
	//
	// A service that already has "Wills - Katoomba"-style children uses those
	// (they are the pages a visitor should open). A service with none falls
	// back to the /locations suburbs, so Power of Attorney still has a list.

	local.pagesSvc  = application.wirebox.getInstance( "PageService@pages" );
	local.published = local.pagesSvc.getPublishedPagesForSite( args.site.getId() );
	local.skip      = "how-it-works,creating-a-will";

	local.byPath           = {};
	local.childrenByParent = {};

	for ( local.p in local.published ) {
		local.byPath[ local.p.getPath() ] = local.p;
		local.pid = val( local.p.getParentId() ?: 0 );
		if ( local.pid ) {
			local.pidKey = toString( local.pid );
			if ( !structKeyExists( local.childrenByParent, local.pidKey ) ) {
				local.childrenByParent[ local.pidKey ] = [];
			}
			arrayAppend( local.childrenByParent[ local.pidKey ], local.p );
		}
	}

	local.legalKids = [];
	if ( structKeyExists( local.byPath, "legal-services" ) ) {
		local.legalKey = toString( local.byPath[ "legal-services" ].getId() );
		if ( structKeyExists( local.childrenByParent, local.legalKey ) ) {
			local.legalKids = local.childrenByParent[ local.legalKey ];
		}
	}

	local.services = [];
	for ( local.child in local.legalKids ) {
		if ( !listFindNoCase( local.skip, local.child.getSlug() ) ) {
			arrayAppend( local.services, local.child );
		}
	}
	arraySort( local.services, function( a, b ){
		var d = val( a.getSortOrder() ) - val( b.getSortOrder() );
		return d != 0 ? d : compareNoCase( a.getTitle(), b.getTitle() );
	} );

	local.fallbackPlaces = [];
	if ( structKeyExists( local.byPath, "locations" ) ) {
		local.locKey = toString( local.byPath[ "locations" ].getId() );
		local.regions = structKeyExists( local.childrenByParent, local.locKey )
			? local.childrenByParent[ local.locKey ]
			: [];
		for ( local.region in local.regions ) {
			local.regionKey = toString( local.region.getId() );
			local.suburbs = structKeyExists( local.childrenByParent, local.regionKey )
				? local.childrenByParent[ local.regionKey ]
				: [];
			for ( local.suburb in local.suburbs ) {
				arrayAppend( local.fallbackPlaces, {
					"label" : local.suburb.getTitle(),
					"href"  : "/" & local.suburb.getPath()
				} );
			}
		}
		arraySort( local.fallbackPlaces, function( a, b ){
			return compareNoCase( a.label, b.label );
		} );
	}

	local.panels = [];
	for ( local.svc in local.services ) {
		local.prefix = local.svc.getTitle() & " - ";
		local.places = [];
		local.svcPrefix = local.svc.getPath() & "/";

		for ( local.p in local.published ) {
			if ( left( local.p.getPath(), len( local.svcPrefix ) ) != local.svcPrefix ) {
				continue;
			}
			if ( left( local.p.getTitle(), len( local.prefix ) ) != local.prefix ) {
				continue;
			}
			if ( structKeyExists( local.childrenByParent, toString( local.p.getId() ) ) ) {
				continue;
			}
			arrayAppend( local.places, {
				"label" : trim( mid( local.p.getTitle(), len( local.prefix ) + 1, 500 ) ),
				"href"  : "/" & local.p.getPath()
			} );
		}

		if ( !arrayLen( local.places ) ) {
			local.places = duplicate( local.fallbackPlaces );
		} else {
			arraySort( local.places, function( a, b ){
				return compareNoCase( a.label, b.label );
			} );
		}

		arrayAppend( local.panels, {
			"slug"   : local.svc.getSlug(),
			"title"  : local.svc.getTitle(),
			"href"   : "/" & local.svc.getPath(),
			"places" : local.places
		} );
	}
</cfscript>
